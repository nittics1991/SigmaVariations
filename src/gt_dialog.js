    ...
    if (buttons.length > 0) {
      ...
      for (var i = 0; i < buttons.length; i++) {
        ...
        if (buttons[i].breakline) {
          ...
          btn = Sigma.$e("button", {
            id: this.domId + "_" + buttons[i].id,
            className: "gt-input-button",
            innerHTML: buttons[i].text
          });
...
Sigma.createColumnTable = function(grid, chkCfg) {
  ...
  function createCheckHD(_cfg) {
    return _cfg.checkType == "checkbox"
      ? '<input type="checkbox" name="' +
          _cfg.name +
          '" class="gt-f-totalcheck" />'
      : '<input type="radio" name="' + _cfg.name + '" />';
  }

  var cl = grid.columnList;
  var h = [
    '<table class="gt-table" style="margin-left:0px" cellSpacing="0"  cellPadding="0" border="0" >',
...
Sigma.createColumnDialog = function(dType, cfg) {
  ...
  var okFn = function() {
    ...
    var tableObj =
      Sigma.U.getTagName(colDiv) == "TABLE"
        ? colDiv
        : colDiv.getElementsByTagName("table")[0];
  ...
  var dialog = new Sigma.Dialog({
    ...
    body: [
      '<div id="' +
        dialogId +
        "_div" +
        '" onclick="Sigma.clickHandler.onTotalCheck(event)" class="gt-column-dialog" >',
      "</div>"
    ].join(""),
    buttons: [
...
Sigma.createColumnSelect = function(grid, id) {
  grid = Sigma.$grid(grid);
  var colSelect = [
    "<select" + (id ? ' id="' + id + '" ' : " ") + ' class="gt-input-select">'
  ];
...
Sigma.createFilterItem = function(grid, cid, cname) {
  ...
  var colInfo =
    '<input type="text" readonly="readonly" class="gt-input-text gt-filter-col-text" value="' +
    cname +
    '" />';
  ...
  var fButton =
    '<button class="gt-input-button gt-filter-del" >' +
    grid.getMsg("TEXT_DEL") +
    "</button>" +
    '<button class="gt-input-button gt-filter-up" >' +
    grid.getMsg("TEXT_UP") +
    "</button>" +
    '<button class="gt-input-button gt-filter-down" >' +
    grid.getMsg("TEXT_DOWN") +
    "</button>";
