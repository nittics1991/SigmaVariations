...
Sigma.GridDefault = {
  ...
  initialize: function(id, options) {
    ...
    this.toolbarContent =
      this.toolbarContent === false ? false : this.toolbarContent;
    ...
    var pageInfo =
      options.pageInfo || (this.dataset ? this.dataset.pageInfo : null) || {};
  ...
  initColumns: function(colsOptions) {
    ...
    this.gridEditorCache =
      this.gridEditorCache || Sigma.$e("div", { className: "gt-editor-cache" });
    ...
    for (var i = 0; i < colNum; i++) {
      ...
      colObj.editor = Sigma.Editor
        ? Sigma.Editor.factroy(colObj.editor, this)
        : null;
      ...
      if (colObj.renderer == "by editor" && colObj.editor) {
        ...
        colObj.renderer = Sigma.U.parseExpression(
          colObj.renderer,
          "record",
          "value,record,col,grid,colNo,rowNo"
        );
      }
  ...
  initHead: function() {
    ...
    this.freezeBodyDiv.style.top =
      this.headDivHeight + this.freezeFixH + 0 + "px";
  },
  ...
  createMain: function() {
    ...
    if (this.isNative) {
      ...
      if (
        ...
        Sigma.U.getTagName(this.container) == "BODY"
      ) {
    ...
    this.gridFilterRSCache =
      this.gridFilterRSCache ||
      Sigma.$e("table", { className: "gt-filter-rs-cache" });
  ...
  createHeader: function() {
    ...
    var tagName = this.headDiv.firstChild
...
**/
  _onKeydown: function(event) {
    ...
    if (kcode == Sigma.Const.Key.ESC) {
      ...
      if (newCell) {
        Sigma.U.stopEvent(event);
        var or = oldCell.parentNode,
          nr = newCell.parentNode;
        //if (or!= nr){
        //this.unselectRow(or);
  ...
  startEdit: function() {
    if (
      ...
      this.activeCell &&
      this.activeEditor &&
      (this.activeColumn.editable || this.isInsertRow(this.activeRow)) &&
      Sigma.$invoke(this.activeColumn, "beforeEdit", [
        this.activeValue,
        this.activeRecord,
        this.activeColumn,
        this
      ]) !== false &&
      Sigma.$invoke(this, "beforeEdit", [
        this.activeValue,
        this.activeRecord,
        this.activeColumn,
        this
  ...
  hideEditor: function() {
    if (this.editing) {
      ...
      var validResult = this.validValue(
        this.activeColumn,
        value,
        this.activeRecord,
        this.activeCell
      ...
      Sigma.$invoke(this.activeColumn, "afterEdit", [
        value,
        oldValue,
        this.activeRecord,
        this.activeColumn,
  ...
  getTwinRows: function(row) {
    var row1 = row,
      rowNo = row.rowIndex;
    var tbody = !row1.id
      ? this.gridTbodyList[0]
      : this.freezeBodyTable.tBodies[0];
  ...
  initActiveObj: function(event, newActiveCell) {
    newActiveCell =
      newActiveCell || Sigma.U.getParentByTagName("td", null, event);
  ...
  initActiveObj_startEdit: function(event, newCell, dbl) {
    ...
    if (
      this.activeEditor &&
      (this.clickStartEdit || (p_activeCell && p_activeCell == this.activeCell))
    ) {
      Sigma.U.stopEvent(event);
      this.syncActiveObj(newCell);
      this.startEdit();
  ...
  _onCellSelect: function(ets) {
    Sigma.$invoke(this, "onCellSelect", [
      this.activeValue,
      this.activeRecord,
      ets.cell,
      ets.row,
  ...
  _onRowSelect: function(row, event) {
    ...
    if (this.multiSelect && event.shiftKey) {
      ...
      if (lastRow && lastRow.parentNode == row.parentNode) {
        this.unselectAllRow();
        var sibling =
          lastRow.rowIndex > row.rowIndex ? "prevElement" : "nextElement";
  ...
  _onCellDblClick: function(event, cell, ets) {
    Sigma.$invoke(this, "onCellDblClick", [
      ets.value,
      ets.record,
      ets.cell,
      ets.row,
  ...
  _onRowDblClick: function(event, row, ets) {
    Sigma.$invoke(this, "onRowDblClick", [
      ets.value,
      ets.record,
      ets.cell,
      ets.row,
  ...
  _onBodyClick: function(event, dbl, bodyEl, ets) {
    ...
    var clickCheckColum =
      Sigma.U.getTagName(et) == "INPUT" && et.className == "gt-f-check";
    if (clickCheckColum) {
  ...
  _eventHandler: function(event, name, el) {
    ...
    switch (name) {
      case "contextmenu":
        Sigma.$invoke(grid, "onContextMenu", [
          ets.value,
          ets.record,
          ets.cell,
          ets.row,
...
**/
  ...
  getSaveParam: function(reqParam) {
    ...
    this.submitUpdatedFields &&
      (reqParam[this.CONST.updatedFields] = this.getUpdatedFields());
    return reqParam;
  },
  ...
  getLoadParam: function(reqParam) {
    ...
    this.submitColumnInfo &&
      (reqParam[this.CONST.columnInfo] = this.getColumnInfo());
  ...
  load: function(recount, force) {
    ...
    if (Sigma.$invoke(this, "beforeLoad", [reqParam, this]) !== false) {
      if (
        !url ||
        (force !== true && this.remotePaging === false && !this.isFirstLoad)
      ) {
        // todo
  ...
  saveCallBack: function(response, reqParam, onNav) {
    ...
    if (this.requesting) {
      ...
      var success = !(
        respD[this.CONST.success] === false ||
        respD[this.CONST.success] === "false"
      );
    ...
    if (this.reloadAfterSave || (this.autoSaveOnNav && onNav)) {
      ...
      if (this.recountAfterSave) {
        ...
        this.getPageInfo().totalRowNum =
          respD[respD.CONST.pageInfo].totalRowNum ||
          this.getPageInfo().totalRowNum;
      }
      this.reload();
    }
  ...
  responseCallBack: function(response, reqParam, action) {
    ...
    response =
      Sigma.$invoke(this, action + "ResponseHandler", [response, reqParam]) ||
      response;
  ...
  update: function(record, fieldName, newValue) {
    if (
      Sigma.$invoke(this, "beforeUpdate", [record, fieldName, newValue]) !==
      false
    ) {
  ...
  renderInsertedRow: function(record) {
    ...
    newTR = Sigma.U.createTrFromHTML(
      this.buildGridRow(trStart, record, this.rowNum, colNum),
      this.gridTable.tFoot
    );
    if (this.showIndexColumn) {
      newFTR = Sigma.U.createTrFromHTML(
        this.buildGridIndexRow(trStart, record, this.rowNum, colNum),
        this.freezeBodyTable.tFoot
      );
    } else {
      newFTR = Sigma.U.createTrFromHTML(
        trStart + "</tr>",
    ...
    newTR.id = rowId;
    newTR.setAttribute(
      Sigma.Const.DataSet.INDEX,
      record[Sigma.Const.DataSet.SN_FIELD]
    );
    return [newTR, newFTR];
  },
  ...
  hasDataRow: function() {
    ...
    var hasNew = !!(
      this.gridTable.tFoot && this.gridTable.tFoot.rows.length > 0
    );
  ...
  buildGridTable: function(grid, tableHTML, freezeTableHTML) {
    ...
    indexColumnWidth =
      (indexColumnWidth < 2 ? 1.5 : indexColumnWidth) * 7 + 2 + 1;
    ...
    this.indexColumnCell = [
      '<td class="gt-index-col" ' +
        tdWidthStyle +
        ' ><div class="gt-inner" ' +
        divWidthStyle +
        " >",
    ...
    if (grid.showIndexColumn) {
      ...
      grid.freezeHeadDiv.style.left = grid.freezeBodyDiv.style.left =
        this.freezeFixW + "px"; //(0-grid.freezeHeadTable.offsetWidth)+"px"; //(0-grid.freezeBodyTable.offsetWidth)+"px";
      grid.headTable.style.marginLeft = grid.tableMarginLeft + "px";
      grid.freezeHeadDiv.style.display = grid.freezeBodyDiv.style.display =
        "block";
      grid.freezeBodyDiv.style.height =
        parseInt(grid.bodyDiv.style.height) + "px";
    } else {
      grid.freezeHeadDiv.style.display = grid.freezeBodyDiv.style.display =
        "none";
    ...
    tableHTML.push(
      Sigma.U.replaceAll(
        Sigma.T_G.tableStartHTML,
        "margin-left:0px",
        "margin-left:" + grid.tableMarginLeft + "px"
      )
    ); //id = grid.id +"_bodyTable"
  ...
  buildGridRow: function(trStart, record, rowNo, colNum, groupInfo) {
    ...
    for (var cn = 0; cn < colNum; cn++) {
      ...
      rowHTML.push(
        Sigma.T_G.cell(
          col,
          col.renderer(record[col.fieldIndex], record, col, this, _cn, rowNo),
          attr
        )
      );
      _cn++;
  ...
  **/
      ...
      for (var i = 0; i < rowNum; i++) {
        ...
        if (colObj.grouped) {
          ...
          if (
            ...
            groupH.attr =
              ' rowspan="' + rspan + '" style="background-color: #eef6ff;"  ';
            rspan = 1;
            groupH = cell;
            groupValue = record[colObj.fieldIndex];
  ...
  createSortInfo: function(colObj) {
    return {
      ...
      getSortValue: colObj.getSortValue,
      sortFn: colObj.sortFn
    };
  },
  ...
  updateEditState: function() {
    var newRecords = this.getInsertedRecords(),
      row,
      record,
      i,
      j,
...
**/
  initToolbar: function() {
    if (
      this.resizable &&
      (this.toolbarPosition == "bottom" || this.toolbarPosition == "b") &&
      this.toolBarBox
    ) {
      ...
      for (var j = 0; j < barbutton.length; j++) {
        ...
        if (b == "|") {
          ...
          if (!this.pageStateBar) {
            this.pageStateBar = Sigma.ToolFactroy.create(
              this,
              "pagestate",
              this.showPageState
            );
          ...
          lastt = this.tools[b + "Tool"] = Sigma.ToolFactroy.create(
            this,
            b,
            this["show" + ub + "Tool"]
          );
  ...
  **/
      ...
      if (this.pageStateBar) {
        ...
        //if (pageInfo.endRowNum - pageInfo.startRowNum < 1) {
          ...
          this.pageStateBar.innerHTML =
            "<div>" +
            Sigma.$msg(
              this.getMsg(pageInput ? "PAGE_STATE" : "PAGE_STATE_FULL"),
              pageInfo.startRowNum,
              pageInfo.endRowNum,
              pageInfo.totalPageNum,
              pageInfo.totalRowNum,
              pageInfo.pageNum
            ) +
            "</div>";
        }
  ...
  createGridMenu: function() {
    ...
    var groupColDialog = !this.allowGroup
      ? null
      : Sigma.createColumnDialog("group", {
          gridId: gridId,
          ...
          checkFn: "group",
          uncheckFn: "ungroup",
          checkType: grid.multiGroup ? "checkbox" : "radio",
          canCheck: function(col) {
            return col.filterable !== false && !col.hidden;
          }
        });

    var freezeColDialog = !this.allowFreeze
      ? null
      : Sigma.createColumnDialog("freeze", {
          gridId: gridId,
          checkValid: function(r) {
            return r.frozen;
          },
    ...
    var hideColDialog = !this.allowHide
      ? null
      : Sigma.createColumnDialog("show", {
          gridId: gridId,
          checkValid: function(r) {
            return !r.hidden;
          },
      ...
      for (var i = 0; i < this.skinList.length; i++) {
        skinItemList.push(
          new Sigma.MenuItem({
            ...
            checked: i === 0,
            onclick: _changeSkin(this.skinList[i].value)
          })
        );
      }
      ...
      [
        ...
        groupColDialog
          ? new Sigma.MenuItem({
              gridId: grid.id,
              type: "",
              text: grid.getMsg("MENU_GROUP_COL"),
        ...
        freezeColDialog
          ? new Sigma.MenuItem({
              gridId: grid.id,
              type: "",
              text: grid.getMsg("MENU_FREEZE_COL"),
              cls: "gt-icon-freeze",
              onclick: function() {
        ...
        hideColDialog
          ? new Sigma.MenuItem({
              gridId: grid.id,
              type: "",
              text: grid.getMsg("MENU_SHOW_COL"),
              cls: "gt-icon-hidecol",
              onclick: function() {
  ...
  getSelectedRecord: function() {
    return (
      this.getRecordByRow(this.selectedRows[this.selectedRows.length - 1]) ||
      this.activeRecord
    );
  },
  ...
  getGridHTML: function(multiPage, gridHTML) {
    ...
    gridHTML = gridHTML || [
      Sigma.T_G.bodyTableStart(this.id, false),
      hdHTML,
      "<tbody>"
    ];
    var startR = 0;
    ...
    gridHTML.push(
      this.getGridBodyHTML(startR, endR, pageSize, gridHTML, null, true)
    );
  ...
  getGridBodyHTML: function(
    ...
    for (var rn = startR; rn < endR; rn++) {
      ...
      if (!isExport) {
        ...
        grid.currentRowNum++;
        if (grid.showIndexColumn) {
          freezeTableHTML.push(
            grid.buildGridIndexRow(trStartS + TRE, record, rn, colNum)
          );
        }
      } else {
      ...
      if (grid.isNextGroup(record, lastRecord, rn)) {
        tableHTML.push(
          grid.buildGroupGridRow(trStartS + TRE, record, rn, colNum)
        );
      }
  ...
  filterHandler: {
    hide: function(filterInfo) {
      ...
      var rs = Sigma.Grid.filterCheck[filterInfo.checkType](
        filterInfo.value,
        filterInfo.checkValue
      );
      if (rs) {
      } else {
      }
    ...
    if (selected) {
      ...
      for (var i = 0, len = rows.length; i < len; i++) {
        ...
        if (!this.isEmptyRow(row)) {
          ...
          var record = this.getRecordByRow(row);
          if (
            Sigma.$invoke(this, "beforeRowSelect", [
              record,
              row,
              rowNo,
              this
  ...
  exportGrid: function(type, fileName, url, name, action) {
    var grid = this;
    if (
      Sigma.$invoke(grid, "beforeExport", [
        type,
        fileName,
        url,
        name,
      ...
      try {
        ...
        name =
          name ||
          this.jsonParamName ||
          (this.ajax ? this.ajax.jsonParamName : Sigma.AjaxDefault.paramName);
  ...
  getRecord: function(rowNoTdTr) {
    ...
    if (Sigma.U.isNumber(rowNoTdTr)) {
      ...
      rowNo =
        this.selectedRows[this.selectedRows.length - 1].getAttribute(
          Sigma.Const.DataSet.INDEX
        ) / 1;
    } else {
      rowNo = 0;
  ...
  insert: function(record, startEdit) {
    record =
      record ||
      this.cloneDefaultRecord() ||
      (this.dataset.getRecordType() == "array" ? [] : {});
  ...
  save: function(onNav) {
    ...
    var hasModified =
      (_insert && _insert.length > 0) ||
      (_update && _update.length > 0) ||
      (_delete && _delete.length > 0);
  ...
  setContent: function(respD) {
    ...
    if (Sigma.$type(respD, "array")) {
      ...
      respD[this.CONST.pageInfo] =
        respD[this.dataPageInfo || this.CONST.pageInfo];
    ...
    if (
      respD[this.dataRoot] &&
      Sigma.$invoke(this, "beforeDatasetUpdate", [respD[this.dataRoot]]) !==
        false
    ) {
      //if (this.remotePaging===false){
      pageInfo.totalRowNum =
        pageInfo.totalRowNum || respD[this.dataRoot].length;
      //}
      ...
      case "filter":
        ...
        grid.filterDialog =
          ...
          Sigma.createFilterDialog({
            title: grid.getMsg("DIAG_TITLE_FILTER"),
            gridId: grid.id
          });
        ...
        break;
      case "chart":
        var record = grid.activeCell
          ? grid.getRecordByRow(grid.activeRow)
          : grid.getRecord();
        if (!record) {
          break;
        }
        ...
        if (record) {
          ...
          grid.chart = new Sigma.Chart({
            swfPath: grid.SigmaGridPath + "/flashchart/fusioncharts/charts/",
            width: w - 3,
            height: h - 23,
          ...
          Sigma.$each(grid.columnList, function(colObj, i) {
            if (colObj.chartCaption) {
              caption = colObj.chartCaption.replace(
                "{@}",
                record[colObj.fieldIndex]
              );
            }
            if (colObj.inChart) {
              date.push([
                colObj.header || colObj.title,
                record[colObj.fieldIndex],
                colObj.chartColor || "66bbff"
              ]);
            }
          });
  ...
  setDimension: function(newWidth, newHeight, isInit) {
    ...
    if (
      (newHeight.indexOf("%") > 1 || newWidth.indexOf("%") > 1) &&
      this.monitorResize
    ) {
  ...
  moveColumn: function(oldIndex, newIndex, ifreeze) {
    ...
    var minCol =
      ifreeze !== true && this.freezeHeadDiv.style.display == "block"
        ? this.frozenColumnList.length
        : 0;
    ...
    Sigma.U.insertNodeBefore(
      this.columnList[oldIndex].headCell,
      this.columnList[newIndex].headCell
    );
    for (i = 0; i < rows.length; i++) {
      Sigma.U.insertNodeBefore(
        rows[i].cells[oldIndex],
        rows[i].cells[newIndex]
      );
    }
  ...
  focusCell: function(cell) {
    if (cell && !this.isGroupRow(cell)) {
      ...
      if (_change) {
        Sigma.U.removeClass(
          this.activeCell,
          "gt-cell-actived" + (this.activeEditor ? "-editable" : "")
        );
        this.syncTwinRowCell(null, this.activeCell);
      }
  ...
  refreshRow: function(row, record) {
    ...
    for (var i = 0; i < row.cells.length; i++) {
      ...
      var changed = false;
      cell.firstChild.innerHTML = colObj.renderer(
        record[colObj.fieldIndex],
        record,
        colObj,
        this,
  ...
  checkRow: function(row, checked) {
    ...
    for (var i = 0; i < row.cells.length; i++) {
      ...
      if (colObj.isCheckColumn) {
        ...
        if (
          Sigma.U.getTagName(el) == "INPUT" &&
          el.type == "checkbox" &&
          Sigma.U.hasClass(el, "gt-f-check")
        ) {
          if (el.checked !== checked) {
            ...
            checked
              ? (this.checkedRows[el.value] = true)
              : delete this.checkedRows[el.value];
          }
          return;
        }
      }
...
Sigma.$extend(Sigma.Grid, {
  ...
  handleOverRowCore: function(evt, grid, row) {
    ...
    if (grid.overRow) {
      grid.overRow.className = grid.overRow.className.replace(
        " gt-row-over",
        ""
      );
    }
  ...
  startGridResize: function(evt, grid) {
    ...
    var ow = grid.gridDiv.offsetWidth,
      oh = grid.gridDiv.offsetHeight;
  ...
  startColumnResize: function(evt, colObj) {
    ...
    grid.separateLineMinX =
      colObj.oldRightX + colObj.minWidth - colObj.headCell.offsetWidth; //colObj.width
  ...
  endDocGridHandler: function(evt, grid) {
    ...
    if (grid.separateLine && grid.separateLine.style.display == "block") {
      ...
      if (
        ...
        if (
          Sigma.$invoke(grid, "beforeColumnMove", [
            oldIndex / 1,
            newIndex / 1,
            grid
          ]) !== false
...
**/
  initColumnEvent: function(grid, colObj, headCell, sortIcon) {
    ...
    Sigma.U.addEvent(headCell, "click", function(event) {
      ...
      if (!grid.isColumnResizing) {
        ...
        if (
          Sigma.U.getTagName(et) == "INPUT" &&
          et.type == "checkbox" &&
          Sigma.U.hasClass(et, "gt-f-totalcheck")
        ) {
          Sigma.checkTotalBox(et, grid, colObj);
        } else if (colObj.sortable && et.className != "gt-hd-button") {
          grid.lastAction = "sort";
          grid.sorting = true;
          var sortOrder =
            colObj.sortOrder == "asc"
              ? "desc"
              : colObj.sortOrder == "desc" && colObj.enableDefaultSort
                ? "defaultsort"
                : "asc";
  ...
  mappingRenderer: function(mapping, defaultText) {
    return function(cellValue) {
      return (
        mapping[cellValue] ||
        (defaultText === undefined || defaultText === null
          ? cellValue
          : defaultText)
      );
    };
  ...
  findGridByElement: function(obj) {
    ...
    while ((obj = obj.parentNode)) {
      if (
        Sigma.U.getTagName(obj) == tagName &&
        Sigma.U.hasClass(obj, className)
      ) {
        gridId = obj.id;
        break;
      }
...
Sigma.initGlobalEvent = function() {
  ...
  Sigma.U.addEvent(d, "click", function(event) {
    Sigma.activeGrid &&
      (Sigma.activeGrid.endEdit() || Sigma.activeGrid.closeGridMenu());
  });
