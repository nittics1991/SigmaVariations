...
*/
  ...
  createGotoPage: function() {
    ...
    if (grid.toolBar) {
      ...
      text1 = Sigma.$e("div", {
        innerHTML: grid.getMsg("PAGE_BEFORE"),
        className: "gt-toolbar-text"
      });
      text2 = Sigma.$e("div", {
        innerHTML: grid.getMsg("PAGE_AFTER"),
        className: "gt-toolbar-text"
      });
  ...
  refreshState: function(pageInfo, doCount) {
    ...
    if (doCount !== false) {
      ...
      pInfo.pageNum =
        pInfo.pageNum > pInfo.totalPageNum ? pInfo.totalPageNum : pInfo.pageNum;
      ...
      pInfo.endRowNum =
        pInfo.endRowNum > pInfo.totalRowNum
          ? pInfo.totalRowNum
          : pInfo.endRowNum;
    }
    return pInfo;
  ...
  refreshNavBar: function(pageInfo) {
    ...
    if (grid && grid.pageSizeSelect) {
      grid.pageSizeList =
        !grid.pageSizeList || grid.pageSizeList.length < 1
          ? [grid.pageSize]
          : grid.pageSizeList;
      grid.pageSizeSelect.innerHTML = "";
      Sigma.U.createSelect(
        Sigma.U.listToMap(grid.pageSizeList),
  ...
  gotoPage: function(navObj, pageNum, lastAction) {
    ...
    pageNum =
      !pageNum || pageNum < 1
        ? 1
        : pageNum > navObj.pageInfo.totalPageNum
          ? navObj.pageInfo.totalPageNum
          : pageNum;

    if (
      Sigma.$invoke(grid, "beforeGotoPage", [
        pageNum,
        oldPageNum,
        navObj,
        grid
  ...
  gotoInputPage: function(event, navObj) {
    navObj.gotoPage(
      navObj,
      Sigma.U.parseInt(navObj.pageInput.value, navObj.pageInfo.pageNum),
      "gotoPage"
    );
  },
  ...
  onclickAction: function(event, itemT) {
    ...
    if (itemT.parentItem) {
      ...
      if (itemT.parentItem.currenItem) {
        Sigma.U.removeClass(
          itemT.parentItem.currenItem.itemBox,
          "gt-menu-activemenu"
        );
      }
    ...
    if (
      itemT.disabled ||
      itemT.onclick.apply(itemT, [event].concat(itemT.onclickArgs || itemT)) ===
        false
    ) {
      Sigma.U.stopEvent(event);
      return;
  ...
  addMenuItems: function(menuItem, position) {
    if (menuItem) {
      if (!this.subMenu) {
        ...
        this.itemAfterIcon &&
          Sigma.U.addClass(this.itemAfterIcon, "gt-menu-parent");
      }
...
Sigma.GridMenu = Sigma.$class({
  ...
  show: function(event) {
    if (
      ...
      this.container.parentNode.className.indexOf("menu") > 1
    ) {
...
Sigma.ToolFactroy = {
  ...
  tools: {
    ...
    pagesize: function(grid) {
      ...
      var text1 = Sigma.$e("div", {
        innerHTML: grid.getMsg("PAGESIZE_BEFORE"),
        className: "gt-toolbar-text"
      });
      var text2 = Sigma.$e("div", {
        innerHTML: grid.getMsg("PAGESIZE_AFTER"),
        className: "gt-toolbar-text"
      });
      grid.toolBar.appendChild(text1);
      grid.toolBar.appendChild(
        Sigma.Button.createToolComp(grid.pageSizeSelect)
      );
      grid.toolBar.appendChild(text2);
