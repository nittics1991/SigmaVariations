...
Sigma.EditorDefault = {
  ...
  initialize: function(options) {
    ...
    this.dom =
      this.dom || Sigma.$e("div", { className: "gt-editor-container" });
...
Sigma.DialogEditor = Sigma.Editor.extend(
  Sigma.$extend(
    {
      getDom: function() {
...
Sigma.$extend(Sigma.Editor, {
  factroy: function(editorInfo, grid) {
    ...
    if (
      editorInfo instanceof Sigma.DialogEditor ||
      editorInfo instanceof Sigma.Dialog
    ) {
    ...
    editorInfo = Sigma.$type(editorInfo, "string")
      ? { type: editorInfo }
      : editorInfo;
    return editorInfo && Sigma.Editor.EDITORS[editorInfo.type]
      ? Sigma.Editor.EDITORS[editorInfo.type](editorInfo)
      : null;
  },
  ...
  EDITORS: {
    ...
    textarea: function(editor) {
      editor = new Sigma.Editor(editor);
      editor.valueDom = Sigma.$e("textarea", {
        style: {
          width: editor.width || "100px",
          height: editor.height || "50px"
        },
        value: editor.defaultValue || "",
        className: "gt-editor-text"
      });
      ...
      editor.getDisplayValue = function(value) {
        ...
        for (var i = 0; i < this.valueDom.options.length; i++) {
          if (String(this.valueDom.options[i].value) === String(value)) {
            return (
              this.valueDom.options[i].text ||
              this.valueDom.options[i].innerHTML
            );
          }
        }
        return this.defaultText || this.defaultText === ""
          ? this.defaultText
          : null;
      };
      return editor;
    ...
    date: function(editor) {
      ...
      var input = Sigma.$e("input", {
        ...
        value: editor.defaultValue || "",
        className: "gt-editor-text",
        style: { width: "78px", styleFloat: "left" }
      });
      var button = Sigma.$e("input", {
        type: "button",
        value: editor.defaultValue || "",
