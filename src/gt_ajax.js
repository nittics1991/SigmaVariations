...
Sigma.Ajax = Sigma.$class({
  ...
  send: function(options) {
    ...
    options = Sigma.$extend(
      { data: this.data, url: this.url, method: this.method },
      options
    );
...
Sigma.JSON = {
  encode: function(obj, cP, format) {
    ...
    switch (Sigma.$type(obj)) {
      case "string":
        return (
          '"' + obj.replace(/[\x00-\x1f\\"]/g, Sigma.JSON.$replaceChars) + '"'
        );
      case "array":
        string = [];
      ...
      case "object":
        ...
        return (
          "{" + br + (format ? string.join("," + br) : string) + br + "}" + br
        );
      case "number":
      case "boolean":
        return String(obj);
