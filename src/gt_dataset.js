...
Sigma.DataSetDefault = {
  ...
  getRecordType: function(recordType, _record) {
    ...
    if (
      !Sigma.$type(this.recordType, "string") &&
  ...
  updateRecord: function(record, fieldName, newValue) {
    ...
    if (
      this.insertedRecords[sn] ||
      Sigma.$invoke(this, "onRecordUpdate", [record, fieldName, newValue]) !==
        false
    ) {
      if (type == "int") {
  ...
  undeleteRecord: function(recordNoOrRecord) {
    ...
    if (Sigma.$type(recordNoOrRecord, "number")) {
      ...
      recordNoOrRecord &&
      (Sigma.$type(recordNoOrRecord, "object") ||
        Sigma.$type(recordNoOrRecord, "array"))
    ) {
  ...
  isInsertedRecord: function(record) {
    return (
      record &&
      this.insertedRecords[record[Sigma.Const.DataSet.SN_FIELD]] == record
    );
  },
    ...
    for (var i = 0; i < sortInfos.length; i++) {
      ...
      if (s) {
        ...
        sortFuncs.push(
          !isDefault && s.sortFn
            ? s.sortFn
            : this.getSortFuns(fieldIndex, s.sortOrder, type, s.getSortValue)
        );
      }
    }
  ...
  getSortFuns: function(fieldIndex, sortOrder, type, getSortValue) {
    ...
    if (!compSort) {
      var getSortValueFn =
        getSortValue && sortOrder.indexOf("def") !== 0
          ? function(r) {
