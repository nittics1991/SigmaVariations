...
Sigma.ColumnDefault = {
  ...
  setWidth: function(newWidth) {
    ...
    Sigma.U.CSS.updateRule(
      this.CLASS_PREFIX + this.styleClass,
      "width",
      newWidth + grid.cellWidthFix + "px"
    );
    Sigma.U.CSS.updateRule(
      this.CLASS_PREFIX + this.innerStyleClass,
      "width",
      newWidth + grid.innerWidthFix + "px"
    );
  },
  ...
  freeze: function(always) {
    ...
    for (i = 0; i < rows.length; i++) {
      this.freezeCell(
        rows[i],
        grid.freezeHeadTable.tBodies[0],
        freezeRows[i],
        i,
        colNo,
        headCellTemplate,
    ...
    for (i = 0; i < rows.length; i++) {
      this.freezeCell(
        rows[i],
        grid.freezeBodyTable.tBodies[0],
        freezeRows[i],
        i,
        colNo,
        cellTemplate,
    ...
    if (!grid.hasIndexColumn) {
      grid.freezeHeadDiv.style.left = grid.freezeBodyDiv.style.left =
        0 - (indexColumnWidth + grid.cellWidthFix) + grid.freezeFixW + "px";
    }
