...
Sigma.Validator = {
  ...
  integer: function(value) {
    return (
      ...
      !isNaN(value / 1) &&
      Sigma.Validator.RegExpLib["integer"].test(Math.abs(value))
    );
  },
  float: function(value) {
    return (
      !isNaN(value / 1) &&
      Sigma.Validator.RegExpLib["float"].test(Math.abs(value))
    );
  },
  ...
  idcard: function(value) {
    if (
      !value ||
      value.length < 15 ||
      !Sigma.Validator.RegExpLib["idcard"].test(value)
    ) {
      return false;
    }
    var birthday;
  ...
  datetime: function(timeValue, format) {
    ...
    for (var ii = 0; ii < timeValue.length; ii++) {
      ...
      var rs =
        Sigma.Validator.date(values[0], fatms[0]) &&
        Sigma.Validator.time(values[1], fatms[1]);
      if (!rs) {
        return false;
      }
    }
