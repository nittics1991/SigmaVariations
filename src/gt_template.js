  ...
  **/
  ...
  Grid: {
    main: function(grid) {
      ...
      var ghtml = [
        grid.toolbarPosition == "top" || grid.toolbarPosition == "t"
          ? '<div id="' +
            gid +
            '_toolBarBox" class="gt-toolbar-box gt-toolbar-box-top" ></div>'
          : "",

        '<div id="' +
          gid +
          '_viewport" class="gt-viewport' +
          (grid.simpleScrollbar ? " gt-simple-scrollbar" : "") +
          '" >',
        '<div id="' +
          gid +
          '_headDiv" class="gt-head-div"><div class="gt-head-wrap" ></div>',
        ...
        grid.toolbarPosition == "bottom" || grid.toolbarPosition == "b"
          ? '<div id="' + gid + '_toolBarBox" class="gt-toolbar-box" ></div>'
          : "",
        '<div id="' + gid + '_separateLine" class="gt-split-line"></div>',

        '<div id="' + gid + '_mask" class="gt-grid-mask" >',
        '<div  id="' + gid + '_waiting" class="gt-grid-waiting">',
        '<div class="gt-grid-waiting-icon"></div><div class="gt-grid-waiting-text">' +
          grid.getMsg("WAITING_MSG") +
          "</div>",
        "</div>",
        '<div class="gt-grid-mask-bg">',
        "</div>",
    ...
    hdToolHTML:
      '<div class="gt-hd-tool" ><span class="gt-hd-icon"></span><span class="gt-hd-button"></span><span class="gt-hd-split"></span></div>',
    ...
    bodyTableStart: function(id, withTbody) {
      return [
        ...
        withTbody === false ? "" : "<tbody>"
      ].join("");
    },

    tableStartHTML:
      '<table class="gt-table" style="margin-left:0px" cellspacing="0"  cellpadding="0" border="0" ><tbody>',
    ...
    innerStart: function(col) {
      return [
        '<div class="gt-inner ' +
          (col.align ? " gt-inner-" + col.align + " " : "") +
          "" /*(col.styleClass?(" "+col.styleClass):'')*/,
        '" >'
      ].join("");
    },
    ...
    cell: function(col, cellHTML, cellAttributes) {
      return [
        "<td ",
        cellAttributes || col.cellAttributes,
    ...
    getColStyleClass: function(grid, colId) {
      return (
        Sigma.Const.Grid.COL_T_CLASSNAME +
        grid.id +
        "-" +
        colId
      ).toLowerCase();
  ...
  Dialog: {
    create: function(options) {
      ...
      var hasCloseButton = Sigma.$chk(options.hasCloseButton)
      ...
      return [
        ...
        '<div id="' +
          id +
          '_dialog_title"  class="gt-dialog-head-text" >' +
          title +
          "</div>",

        '<div class="gt-dialog-head-button"  >',
        hasCloseButton
          ? '<a href="#" onclick="Sigma.$grid(\'' +
            gid +
            "').closeDialog();return false;\">&#160;</a>"
          : "",
        "</div>",
