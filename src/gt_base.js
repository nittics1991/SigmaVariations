...
Sigma.$extend(Sigma, {
  ...
  $array: function(iterable, start, end, isDeep) {
    ...
    if (iterable) {
      ...
      if (
        Sigma.$type(iterable, "arguments", "collection") ||
        (Sigma.$type(iterable, "array") && (start > 0 || end < iterable.length))
      ) {
  ...
  $msg: function(msgTemplate, msgs) {
    for (var i = 1; i < arguments.length; i++) {
      msgTemplate = Sigma.U.replaceAll(
        msgTemplate,
        "#{" + i + "}",
        arguments[i]
      );
  ...
  $each: function(iterable, fn, bind, arg) {
    var resultList = [];
    if (
      Sigma.$type(iterable, "array", "arguments", "collection") ||
      (iterable &&
        !Sigma.$type(iterable, "string") &&
        Sigma.$type(iterable.length, "number"))
    ) {
      for (var i = 0, j = iterable.length; i < j; i++) {
        resultList.push(
          fn.call(bind || iterable, iterable[i], i, iterable, arg)
        );
      }
    } else {
      for (var name in iterable) {
        resultList.push(
          fn.call(bind || iterable, iterable[name], name, iterable, arg)
        );
      }
    }
    return resultList;
...
Sigma.$byName = function(el) {
  ...
  for (var i = 0; i < elColl.length; i++) {
    el = elColl[i];
    elList.push(
      Sigma.U.contains(["object", "embed"], el.tagName.toLowerCase())
        ? el
        : Sigma.$e(el)
    );
  }
...
Sigma.Utils = {
  ...
  toggleClass: function(el, className) {
    return Sigma.U.hasClass(el, className)
      ? Sigma.U.removeClass(el, className)
      : Sigma.U.addClass(el, className);
  },

  hasSubString: function(str, string, s) {
    return s
      ? (s + str + s).indexOf(s + string + s) > -1
  ...
  createSelectHTML: function(map, defaultValue, opt) {
    ...
    var selectH = [
      "<select" + id + st + 'class="gt-input-select ' + cls + '">'
    ];
    for (var k in map) {
      var s = "";
  ...
  getEventTarget: function(evt) {
    ...
    return !targetEl
      ? null
      : targetEl.nodeType == 3 ? targetEl.parentNode : targetEl;
  },
  ...
  getBorderWidths: function(el) {
    return [
      Sigma.U.parseInt(el.style.borderTopWidth),
  ...
  getPaddings: function(el) {
    return [
      Sigma.U.parseInt(el.style.paddingTop),
  ...
  getPageScroll: function() {
    var dd = Sigma.doc.documentElement,
      db = Sigma.doc.body;
  ...
  replaceAll: function(exstr, ov, value) {
    ...
    var r = null;
    var cmd =
      "r=exstr.replace(" + rep + "," + Sigma.U.escapeString(value) + ")";
    eval(cmd);
    return r;
  },
